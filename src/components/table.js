import page from './pics/page1.png';
import dots from "./pics/3dots.png"

const table = () => {
    return (
        <table id="lists1" className="lists">

	<tr className="list-header">
		<th>DOCUMENT TITLE</th>
		<th>STAGES</th>
		<th>OWNED BY</th>
		<th>LAST EDITED</th>
		<th></th>
		<th></th><th></th>

	</tr>
	<tr className="list1">
		<td className="first">
			<div className="img11"><img src={page}/></div>
			<div>Proxy Generation Documentation\</div>
		</td>
		<td className="second grey-box"><mark>Development</mark></td>
		<td className="second">Veneela Nagabandi</td>
		<td className="second">09/20/2020, 06:07am</td>
		
		<td className="green-dot1"></td>
	
		<td className="second ">Published</td>
		<td><img className="Tdot" src={dots}/>
		</td>
		
		
			
		
	</tr>
	
<tr className="list2">
	<td className="first">
	<div className="img11"><img src={page}/></div>
	<div>proxygen release notes </div>
	</td>	
	<td className="second">...</td>
	<td className="second">Veneela Nagabandi</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="green-dot1"></td>
	<td className="second">Published</td>
	<td><img className="Tdot" src={dots}/>
	</td>
</tr>

<tr className="list3">
	<td className="first">
		<div className="img11"><img src={page}/></div>
		<div>API Security Workshop</div>
	</td>
	<td className="second grey-box"><mark>Design</mark></td>
	<td className="second">ER, Vinay</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="green-dot1"></td>
	<td className="second ">Published</td>
	<td><img className="Tdot" src={dots}/>
	</td>

</tr>

<tr className="list4">
	<td className="first">
		<div className="img11"><img src={page}/></div>
		<div>Doc under test</div>
	</td>
	<td className="second">...</td>
	<td className="second">Govindaraj, Santhosh</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="green-dot1"></td>
	<td className="second">Published</td>
	<td><img className="Tdot" src={dots}/>
	</td>


</tr>


<tr className="list5">
	<td className="first">
		<div className="img11"><img src={page}/></div>
		<div>Common 401 Errors</div>
	</td>
	<td className="second">...</td>
	<td className="second">Govindaraj, Santhosh</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="green-dot1"></td>
	<td className="second">Published</td>
	<td><img className="Tdot" src={dots}/>
	</td>

</tr>


<tr className="list6">
	<td className="first">
		<div className="img11"><img src={page}/></div>
		<div>Using API Explorer</div>
	</td>
	<td className="second">...</td>
	<td className="second">Govindaraj, Santhosh</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="pink-dot"></td>
	<td className="second change">Changes Requested</td>
	<td><img className="Tdot" src={dots}/>
	</td>

</tr>

<tr className="list7">
	<td className="first">
		<div className="img11"><img src={page}/></div>
		<div>Apigee Orgs / Envs</div>
	</td>
	<td className="second grey-box"><mark>Development</mark></td>
	<td className="second">Govindaraj, Santhosh</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="green-dot1"></td>
	<td className="second">Published</td>
	<td><img className="Tdot" src={dots}/>
	</td>
</tr>


<tr className="list8">
	<td className="first">
		<div className="img11"><img src={page}/></div>
		<div>Common Gitlab Pipeline Errors</div>
	</td>
	<td className="second grey-box"><mark>Development</mark></td>
	<td v>Govindaraj, Santhosh</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="green-dot1"></td>
	<td className="second">Published</td>
	<td><img className="Tdot" src={dots}/>
	</td>

</tr>


<tr className="list9">
	<td className="first">
		<div className="img11"><img src={page}/></div>
		<div>Apigee UI Access</div>
	</td>
	<td className="second grey-box"><mark>Development</mark></td>
	<td className="second">Kumar, Himanshu</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="green-dot1"></td>
	<td className="second">Published</td>
	<td><img className="Tdot" src={dots}/>
	</td>

</tr>


<tr className="list10">
	<td className="first">
		<div className="img11"><img src={page}/></div>
		<div>Aboard FAQ</div>
	</td>
	<td className="second">...</td>
	<td className="second">ER, Vinay</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="green-dot1"></td>
	<td className="second">Published</td>
	<td><img className="Tdot" src={dots}/>
	</td>

</tr>

<tr className="list11">
	<td className="first">
		<div className="img11"><img src={page}/></div>
		<div>API Config Tool FAQ</div>
	</td>
	<td className="second">...</td>
	<td className="second">Veneela Nagabandi</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="grey-dot"></td>
	<td className="second">Draft</td>
	<td><img className="Tdot" src={dots}/>
	</td>

</tr>


<tr className="list12">
	<td className="first">
		<div className="img11"><img src={page}/></div>
		<div>API Security</div>
	</td>
	<td className="second grey-box"><mark>Design</mark></td>
	<td className="second">Veneela Nagabandi</td>
	<td className="second">09/20/2020, 06:07am</td>
	<td className="grey-dot"></td>
	<td className="second">Draft</td>
	<td><img className="Tdot" src={dots}/>
	</td>
	
</tr>


</table>
    )
}

export default table
